...
三个错误处理函数（KV 未绑定、D1 未绑定、密码未配置）中嵌入的完整 HTML 页面被提取为共享 `renderErrorPage` 渲染器，消除三份重复的 CSS 模板和 HTML 结构。

- 代码行数：从 ~680 行降至 ~48 行，减少约 93%
- 移除 Google Fonts 外部 CDN 请求（3 次），改用系统字体栈
- CSS 压缩为单行，功能完全保留
